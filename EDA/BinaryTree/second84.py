...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        auxL = []
        self._removeOutsideRange(self._root, auxL, min, max)
        return auxL

    def _removeOutsideRange(self, node, auxL, min, max):
        if node is None:
            return

        self._removeOutsideRange(node.left, auxL, min, max)
        if node.left is None and node.right is None:
            if not min <= node.elem <= max:
                auxL.append(node.elem)
        self._removeOutsideRange(node.right, auxL, min, max)
